...
import { Action, ActionService, activeStates, ButtonOptions, ComponentHelper, CustomDialogHelper, DropDownListOptions, GridListHelper, ListViewBase, UserProfileService, userRoles } from 'src/app/shared';
import MessagesService from '../../messages.service';
import { BusinessLineDetailComponent } from '../detail';
...
export class BusinessLinesListComponent extends ListViewBase implements OnInit {

  @ViewChild('componentContainer', { static: false, read: ViewContainerRef }) componentContainer: ViewContainerRef;
  @ViewChild('grdBusinessLines', { static: false }) grdBusinessLines: DxDataGridComponent;
  ...
  public addLineComponentRef: ComponentRef<BusinessLineDetailComponent>;
  public dtsBusinessLines: any;
  public cnmActionsVisible = false;
  public cnmActionsTarget = '';
  public grdBusinessLinesSelectedRowKeys: any[] = [];
  public grdLineArchivedColumnVisible: boolean = false;
  public cnmBatchActionsVisible = false;
  public currentLineDetail: any;
  public btnAddLineVisible = false;
  ...
  constructor(public gridListHelper: GridListHelper,
    ...
    private service: BusinessLineService,
  ...
  private async getBusinessLines(callbackFn: any = null) {
    super.toggleLoadingPanel(true);
    const param = {
      id: this.userProfileService.roleId === userRoles.agency ? this.userProfileService.id : this.userProfileService.agencyId,
  ...
  private initActions() {
    super.getActions(this.userProfileService, this.route, () => {
      ...
      this.btnAddLineVisible = this.hasCreateRight;
  ...
  private getArchiveActionText = (): string => {
    if (this.isBatchAction) {
      const archiveItems = this.grdBusinessLines.instance.getSelectedRowsData()
        .filter(p => p.isArchived);
      const restoreItems = this.grdBusinessLines.instance.getSelectedRowsData()
        .filter(p => !p.isArchived);
      if (archiveItems.length === this.grdBusinessLinesSelectedRowKeys.length) {
        return super.getArchiveText(true);
      }
      if (restoreItems.length === this.grdBusinessLinesSelectedRowKeys.length) {
        return super.getArchiveText(false);
      }
      return super.getArchiveText(null);
    } else {
      return super.getArchiveText(this.currentLineDetail.isArchived);
    }
  }
  ...
  private getDeleteActionVisibleStatus = (): boolean => {
    if (this.currentLineDetail.isArchived && this.hasDeleteRight) {
      return true;
    } else {
      return false;
    }
  ...
  private getArchiveActionVisibleStatus = (): boolean => {
    if (this.isBatchAction) {
      ...
      const restoreItems = this.grdBusinessLines.instance.getSelectedRowsData()
        .filter(p => !p.isArchived);
      if (archiveItems.length === this.grdBusinessLinesSelectedRowKeys.length && this.hasEditRight) {
        return true;
      }
      if (restoreItems.length === this.grdBusinessLinesSelectedRowKeys.length && this.hasEditRight) {
        return true;
      }
      return false;
    }
  ...
  public onGrdBusinessLinesContentReady(e) {
    super.toggleLoadingPanel(false);
  }

  public onGrdBusinessLinesRowClick(e) {
    this.gridListHelper.onDoubleClick(e);
  }

  public onGrdBusinessLinesToolbarPreparing(e: any) {
  ...
  private onDdlArchiveValueChange = (args) => {
    ...
    this.getBusinessLines();
    if (args.value === 'All') {
      this.grdLineArchivedColumnVisible = true;
    } else {
      this.grdLineArchivedColumnVisible = false;
    }
  }
  ...
  private onBtnExportToPDFClick = () => {
    ...
    exportDataGridToPdf({
      jsPDFDocument: doc,
      component: this.grdBusinessLines.instance
    }).then(() => {
      doc.save('BusinessLines.pdf');
    })
  }
  private onBtnBatchActionClick = () => {
    this.isBatchAction = true;
    if (this.grdBusinessLinesSelectedRowKeys.length === 0) {
  ...
  public onCnmBatchActionsItemClick(e: any) {
    super.toggleLoadingPanel(true);
    const selectedItems = this.grdBusinessLinesSelectedRowKeys;
  ...
  public onCnmActionsItemClick(e) {
    ...
    if (e.itemData.code === 'Delete') {
      this.confirmDelete(this.currentLineDetail.id);
    }
    if (e.itemData.code === 'Archive') {
      this.archive([this.currentLineDetail.id]);
    }
  ...
  private archive(selectedItems: any[]) {
    this.service.archive({
      ...
      this.grdBusinessLinesSelectedRowKeys = [];
      this.initPageData();
    });
  }
  ...
  public onBtnBusinessLineClicked() {
    this.isDataProcessing = true;
    this.showLineDetail(null, 'create');
  }
  public onBtnMainActionClick(entity) {
    super.toggleLoadingPanel(true);
    this.isDataProcessing = true;
    this.showLineDetail(entity, 'edit');
  }

  private showLineDetail(entity, mode) {
    this.addLineComponentRef =
      ComponentHelper.addComponent(this.componentFactoryResolver, this.componentContainer, BusinessLineDetailComponent);
    const detailInstance = this.addLineComponentRef.instance;
    detailInstance.hasCreateRight = this.hasCreateRight;
    detailInstance.hasEditRight = this.hasEditRight;
    if (mode === 'create') {
      detailInstance.popupTitle = "Business Line";
      detailInstance.addBusinessLine();
    }
    if (mode === 'edit') {
      detailInstance.popupTitle = "Business Line - " + entity.name;
      detailInstance.editBusinessLine(entity);
    }
    detailInstance.popupHiddenEvent.subscribe(response => {
      ...
      ComponentHelper.removeComponent(this.componentContainer, this.addLineComponentRef);
    });
  }
