...
namespace auto_pilot.services.Services
{
    public class BusinessTypeService : IBusinessTypeService
    {
        ...
        public BusinessTypeService(IMapper mapper, AutoPilotContext context)
        {
            _mapper = mapper;
            _context = context;
        }
        ...
        {
            var businessTypes = _context.BusinessTypes.Where(flt => flt.AgencyId == Id).Select(ps => ps.IsArchived).ToList();
            TilesDTO companyTileDTO = new TilesDTO()
            {
                NumOfActive = businessTypes.Count(flt => !flt),
                NumOfInactive = businessTypes.Count(flt => flt),
                NumOfTotal = businessTypes.Count()
            };
        ...
        public async Task<List<LookupOutputDTO>> GetByFilter(string activeState, long Id)
        {
            ...
            outputDTO = await (from BT in _context.BusinessTypes.Where(flt => flt.AgencyId == Id)
        ...
        {
            var result = await _context.BusinessTypes.Where(flt => flt.Id == Id).FirstOrDefaultAsync();
            return _mapper.Map<LookupOutputDTO>(result); ;
        }

        public async Task<LookupOutputDTO> Create(LookupInputDTO inputDTO)
        {
            var entity = _mapper.Map<BusinessType>(inputDTO);
            entity.CreatedDate = DateTime.Now;
            await _context.BusinessTypes.AddAsync(entity);
        ...
        {
            var entity = await _context.BusinessTypes.FirstOrDefaultAsync(x => x.Id == inputDTO.Id);
            var mapped = _mapper.Map<LookupInputDTO, BusinessType>(inputDTO, entity);
        ...
        {
            var entity = await _context.BusinessTypes.Where(x => x.Id == deleteDTO.Id).FirstOrDefaultAsync();
        ...
        public ValidationResultDTO Check(CheckInputDTO checkInputDTO)
        {
            ...
            var result = _context.AvailableMarkets.Where(x => x.BusinessTypeId == (long)checkInputDTO.Id).ToList();
            if (result.Count > 0)
            {
        ...
        public async Task<ValidationResultDTO> Validate(ValidationDTO validationDTO)
        {
            ...
            var result = await _context.BusinessTypes.Where(x => x.Id != validationDTO.Id && x.Title == validationDTO.Title && x.AgencyId == validationDTO.AgencyId).ToListAsync();
            if (result.Count > 0)
            {
                validationResultDTO.IsValid = false;
            }
        ...
        public ResultDTO Archive(ArchiveInputDTO inputDTO)
        {
            ...
            {
                var entity = _context.BusinessTypes.Where(flt => inputDTO.Ids.Contains(flt.Id)).ToList();
                entity.ForEach(flt =>
                {
